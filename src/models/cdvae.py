...
from src.models.modules.fault_adapter import MLPFaultAdapter, TriangularLinearFaultAdapter
...
class CDVAE(BRCausalModel):
    ...
    def _init_industrial_modules(self):
        ...
        if self.fault_adapter_type == "mlp":
            self.fault_adapter = MLPFaultAdapter(
                input_dim=adapter_input_dim,
                hidden_dim=self.health_state_dim,
                output_dim=self.health_state_dim,
            )
        elif self.fault_adapter_type == "tri_linear":
            self.fault_adapter = TriangularLinearFaultAdapter(
                latent_dim=self.fault_state_dim,
                output_dim=self.health_state_dim,
            )
        else:
            raise ValueError(f"Unsupported fault_adapter_type: {self.fault_adapter_type}")
    ...
    def _run_industrial_dynamics(
        ...
        for t in range(seq_len):
            ...
            if self.fault_adapter_type == "tri_linear":
                delta_t = self.fault_adapter(s_latent[:, t, :])
            else:
