...
class Inference_Net(nn.Module):
    ...
    - h0_*: sequence-level healthy dynamics initial-state posterior
    - s_*: time-varying fault/intervention latent posterior

    Compatibility mapping:
    - re_* aliases to b_* so existing CDVAE fallback path can keep working.
    """
    ...
    def __init__(
        ...
        # Backward-compatible aliases used in legacy call sites
    ...
    def forward(
        ...
        h0_loc = self.fc_h0_mu(last_feat)
        h0_logvar = self.fc_h0_var(last_feat)
        ...
        output: Dict[str, torch.Tensor] = {
            ...
            "s": s,
            "enc_feat": enc_feat,
            # fallback compatibility for legacy RE path
            "re_loc": b_loc,
            "re_logvar": b_logvar,
            "re_sample": b,
        }
